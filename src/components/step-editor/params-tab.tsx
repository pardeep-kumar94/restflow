...
export function ParamsTab({ params, onChange, stepId }: ParamsTabProps) {
  const [entries, setEntries] = useState<Entry[]>(() => toEntries(params));
  const selfUpdate = useRef(false);

  const parentKeys = Object.keys(params).sort().join(",");
  useEffect(() => {
    if (selfUpdate.current) {
      selfUpdate.current = false;
      return;
    }
    setEntries(toEntries(params));
  }, [parentKeys]);
